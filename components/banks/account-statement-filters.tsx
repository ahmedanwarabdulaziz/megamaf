...
export function AccountStatementFilters({
  ...
  const handleSearch = () => {
    ...
    router.push(`/banks/${accountId}/statement?${params.toString()}`);
  };
  ...
  return (
    <div className="bg-muted/30 p-4 rounded-lg border shadow-sm flex flex-wrap gap-4 items-end mb-4">
      ...
      <div className="flex items-center gap-2 mb-2 px-2">
        <input
          type="checkbox"
          id="showAllAccountStmt"
          checked={isAll}
          onChange={e => setIsAll(e.target.checked)}
          className="w-4 h-4 rounded border-gray-300 text-primary"
        />
        <label htmlFor="showAllAccountStmt" className="text-sm font-medium">عرض الكل (بدون تاريخ)</label>
      </div>
