...
export const getProfile = cache(async () => {
  ...
  const { data: profile } = await supabase
    .from("employees")
    .select(
      "id, full_name, role, is_super_admin, can_approve, has_custody_access, has_expense_funding_access, is_active, active_session_id, employee_page_access(page_slug, access_level)"
    )
