-- 0054_fix_project_financial_position.sql
-- Restores missing breakdown columns lost in 0053 and fixes owner_total_collected
-- to sum directly from ledger_entries so unallocated advance payments are included.
...
SELECT
    ...
    -- OWNER CASH IN (now using ledger_entries so unallocated advances count)
    (
    ...
    -- VENDOR/EXPENSE BREAKDOWNS (restored from 0049)
    ...
    COALESCE(vp.paid_in_system, 0) + COALESCE(vcz.opening_paid, 0) AS vendor_claims_paid,
