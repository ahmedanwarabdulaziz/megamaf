...
--
-- Root-cause fix for a real production incident: a vendor payment for
-- السيد نجار اكتوبر (20,000 EGP, 2026-08-27) was recorded twice, ~2 minutes
-- apart, same bank account/project/memo, same user. The client already
-- disables the Save button while a submission is in flight (calculator.tsx),
-- but that guard lives in React state — a full page refresh (which is
-- exactly what a user does when a slow save makes them think it failed)
-- wipes it, and the resubmitted form creates a second, indistinguishable
-- ledger entry. That can only be caught server-side.
--
-- Fix: both vendor-payment RPCs now reject an insert that matches one the
-- SAME employee just recorded (same vendor, amount, funding source, project,
-- memo) within the last 3 minutes, instead of silently creating a duplicate.
-- 3 minutes comfortably covers a refresh-and-resubmit while being far too
-- short to ever collide with a genuine second same-amount payment made later
-- the same day.
...
BEGIN
    ...
    -- Duplicate-submission guard — see migration header.
...
-- Latest definition before this fix: 20260714090000_direct_expense.sql
