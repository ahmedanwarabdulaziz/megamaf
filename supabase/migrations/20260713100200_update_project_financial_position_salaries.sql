-- 20260713100200_update_project_financial_position_salaries.sql
--
-- Folds salary cost into v_project_financial_position, the same way
-- expenses_agg/expenses_paid_agg already do for employee expenses. Salary
-- cost counts on an accrual basis (payslip status IN ('approved','paid')),
-- matching how expenses_agg counts status='approved' regardless of
-- settlement. Loan repayments are NOT included here — a loan is an employee
-- liability, not a project cost.
--
-- This is CREATE OR REPLACE (no DROP), matching how 0057_fix_employee_expenses_paid.sql
-- itself was applied — the two new output columns are appended at the end so
-- no existing column position/type changes, avoiding any CASCADE risk.
...
supplier_payments AS (
    ...
    WHERE le.counterparty_type = 'vendor'
      AND le.direction = 'out'
      AND le.project_id IS NOT NULL
      AND v.kind = 'vendor' -- Fix: Suppliers have kind = 'vendor'
    GROUP BY le.project_id
),
vendor_payments AS (
    SELECT
...
SELECT
    ...
    (
        ...
        (COALESCE(va.vendor_billed, 0) + COALESCE(ia.invoice_total, 0) + COALESCE(ea.total_employee_expenses, 0) + COALESCE(pva.total_prior_certified, 0) + COALESCE(sa.total_salary_cost, 0))
    ) AS balance,
