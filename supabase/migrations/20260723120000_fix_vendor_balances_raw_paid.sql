-- 20260723120000_fix_vendor_balances_raw_paid.sql
--
-- v_vendor_balances.total_paid was computed purely from payment_allocations
-- (via v_claim_paid), so any payment left partially or fully unallocated to a
-- specific claim/invoice/retention was invisible to "المدفوع" on the treasury
-- payables page — even though the money had actually left the treasury.
--
-- Confirmed live on 2026-07-23: vendor احمد ابو هيثم had a 20,050 payment
-- (2026-07-23) with only 4,496 allocated to a claim, leaving 15,554
-- unaccounted for. total_paid showed 281,087 instead of the real 296,641.
--
-- Fix: base total_paid on the RAW ledger amount actually paid out (all
-- ledger_entries where counterparty_type='vendor' AND direction='out'),
-- same approach the "0057_fix_vendor_balances_payments.sql" migration
-- intended — that file was authored but never applied to this database
-- (confirmed via pg_get_viewdef: live view still matched 0050/0052's
-- allocation-based formula). This migration reapplies that fix while
-- preserving the CURRENT live column order (vendor_id, vendor_name,
-- gross_total, total_retention_held, total_due, total_paid, balance) since
