-- 20260713100100_add_salary_ledger_rpcs.sql
--
-- Ledger category additions + payroll/loan lifecycle RPCs. All ledger_entries
-- inserts in this app go through SECURITY DEFINER functions (never a raw
-- client-side .insert()), so all salary/loan cash movement lives here,
-- mirroring disburse_custody and record_vendor_payment_from_expense.
...
-- ============================================================================
-- 2. Broaden v_expense_vendor_paid (already live from
--    20260712100000_vendor_payment_from_expense.sql) to also count salary
--    payments funded from an expense — an expense's unused balance must
--    reflect either use, since the money can only be spent once.
-- ============================================================================
...
LEFT JOIN public.ledger_entries le
   ...
   AND le.category IN ('vendor_payment', 'salary_payment')
GROUP BY e.id;

-- ============================================================================
-- 3. v_payslip_paid — how much of a payslip's net pay has been settled so
--    far, across any number of partial bank/expense payment actions.
-- ============================================================================

CREATE OR REPLACE VIEW public.v_payslip_paid WITH (security_invoker = true) AS
SELECT payslip_id, COALESCE(SUM(amount), 0) AS paid_amount
FROM public.payslip_payments
GROUP BY payslip_id;

-- ============================================================================
-- 4. generate_payroll_run — creates the monthly batch and one payslip per
--    active employee with an open salary row. Each payslip's project split
--    is copied forward from that employee's most recent prior payslip (if
--    any) as an editable starting point — project assignment is a monthly
--    task, not tied to the salary record.
-- ============================================================================

-- create_payslip_for_employee is the shared "build one payslip" step used by
-- both generate_payroll_run (a whole run at once) and
-- add_employee_to_payroll_run (one employee added to an already-generated,
-- still-draft run) — same base-salary lookup + copy-forward-allocations
-- logic either way.
CREATE OR REPLACE FUNCTION public.create_payslip_for_employee(
    p_run_id uuid,
    p_employee_id uuid,
    p_period_year int,
    p_period_month int
...
BEGIN
    ...
    IF v_base_amount IS NULL THEN
        RAISE EXCEPTION 'Employee has no active salary for %-%', p_period_year, p_period_month;
    END IF;

    INSERT INTO public.payslips (payroll_run_id, employee_id, base_amount, gross_amount, net_amount, status)
    VALUES (p_run_id, p_employee_id, v_base_amount, v_base_amount, v_base_amount, 'draft')
    RETURNING id INTO v_payslip_id;

    SELECT p2.id INTO v_prior_payslip_id
    FROM public.payslips p2
    JOIN public.payroll_runs r2 ON r2.id = p2.payroll_run_id
    WHERE p2.employee_id = p_employee_id
      AND (r2.period_year, r2.period_month) < (p_period_year, p_period_month)
    ORDER BY r2.period_year DESC, r2.period_month DESC
    LIMIT 1;

    IF v_prior_payslip_id IS NOT NULL THEN
        FOR v_alloc IN
            SELECT project_id, allocation_type, allocation_value
            FROM public.payslip_project_allocations
            WHERE payslip_id = v_prior_payslip_id
        LOOP
            v_allocated := CASE
                WHEN v_alloc.allocation_type = 'percentage' THEN round(v_base_amount * v_alloc.allocation_value / 100, 2)
                ELSE v_alloc.allocation_value
            END;
            IF v_allocated > 0 THEN
                INSERT INTO public.payslip_project_allocations (payslip_id, project_id, allocation_type, allocation_value, allocated_amount)
                VALUES (v_payslip_id, v_alloc.project_id, v_alloc.allocation_type, v_alloc.allocation_value, v_allocated);
            END IF;
        END LOOP;
    END IF;
...
-- ============================================================================
-- 4b. add_employee_to_payroll_run — adds one more employee's payslip to an
--     already-generated run (e.g. their salary was set after the run was
--     created, or they were missed). Only while the run is still 'draft'.
-- ============================================================================
...
-- ============================================================================
-- 4c. remove_payslip_from_run — deletes one employee's payslip from a run
--     that's still in draft (e.g. added by mistake). Its components and
--     project allocations cascade automatically; a draft payslip never has
--     payslip_payments/loan_repayments rows yet (those only exist once
--     approved/paid), so this is always safe while still draft.
-- ============================================================================
...
-- ============================================================================
-- 5. approve_payroll_run — validates every payslip's project split adds up
--    to its base salary (blocking approval otherwise), then settles loan
--    deductions now that the numbers are final (capacity-capped, any
--    shortfall stays pending on its installment for a future approval).
-- ============================================================================
...
-- ============================================================================
-- 6. pay_payslip_from_bank / pay_payslip_from_expense — partial, repeatable
--    payment actions against an approved payslip, mirroring
--    record_vendor_payment / record_vendor_payment_from_expense exactly: one
--    funding source per action, callable multiple times until the payslip's
--    net_amount is fully covered (tracked via payslip_payments/v_payslip_paid,
--    not a single flag). The funding expense may belong to ANY employee, not
--    just the payslip's own — same as vendor payments.
-- ============================================================================
...
BEGIN
    ...
    SELECT employee_id, status, amount INTO v_expense_employee_id, v_expense_status, v_expense_amount
...
-- ============================================================================
-- 7. disburse_loan — paired bank/employee ledger entries, mirrors
--    disburse_custody, plus building the installment schedule for whichever
--    repayment_type was chosen.
-- ============================================================================
...
-- ============================================================================
-- 8. cancel_loan — v1 scope: only allowed with zero repayments made so far.
-- ============================================================================
