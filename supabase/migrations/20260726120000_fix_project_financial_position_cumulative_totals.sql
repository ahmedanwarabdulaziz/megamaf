-- 20260726120000_fix_project_financial_position_cumulative_totals.sql
--
-- FIX (not yet applied — reviewed with the user before running):
--
-- v_project_financial_position's `total_income` / `total_expenses` / `balance`
-- were built from `owner_billed` / `vendor_billed`, which sum
-- `v_claim_totals.total_due_this_claim` — the INCREMENTAL amount added by only
-- the LATEST approved claim per party. Once a vendor/owner has more than one
-- approved claim, every earlier claim's amount is silently dropped from the
-- project totals (only the newest claim's delta survives).
--
-- Example seen in production: vendor with claims #0..#3 has a true cumulative
-- payable of 424,557.50, but the old formula only counted claim #3's
-- increment over claim #2 (71,769), because it summed the *latest claim per
-- vendor* instead of each vendor's *cumulative* total.
--
-- The same CTEs already compute the correct cumulative figures
-- (`gross_in_system` / `payable_in_system`, exposed as `vendor_claims_gross`
-- / `owner_claims_gross` / etc.) — this migration switches `total_income`,
-- `total_expenses`, `balance`, and the separately-exposed `vendor_claims_billed`
-- column to use those cumulative sums instead.
--
-- Secondary fix: `total_expenses` never included the generic opening-balance
-- `prior_expenses` figure (`project_opening_balances.prior_expenses`) at all —
-- only the per-vendor `prior_vendor_certified` was folded in. This adds it.
--
-- This mirrors the client-side workaround already shipped in
-- app/(app)/projects/_components/project-card.tsx (ownerGross/vendorGross),
-- which worked around this exact bug without anyone fixing the view itself.
-- Applying this migration makes every consumer of the view (project overview
-- tab, project report, dashboards) correct at the source instead of requiring
-- each screen to re-derive the numbers client-side.
--
-- Scope: only touches total_income / total_expenses / balance /
-- vendor_claims_billed. All other exposed columns (owner_claims_gross,
-- vendor_claims_gross/payable/retained, *_paid, current_retention_held,
-- inventory_asset_value, prior_* fields) were already correct and are
-- untouched — this is CREATE OR REPLACE, no column removed or retyped.
...
vendor_claims_agg AS (
    SELECT
        project_id,
        SUM(gross_in_system)      AS vendor_billed, -- FIX: cumulative gross, not latest-claim increment
...
owner_claims_agg AS (
    SELECT
        project_id,
        SUM(gross_in_system)    AS owner_billed, -- FIX: cumulative gross, not latest-claim increment
...
expenses_paid_agg AS (
    SELECT project_id, SUM(settled_amount) AS total_employee_expenses_paid
    FROM public.expenses
    WHERE status = 'approved'
      AND project_id IS NOT NULL
    GROUP BY project_id
...
salaries_agg AS (
    SELECT ppa.project_id, SUM(ppa.allocated_amount) AS total_salary_cost
    FROM public.payslip_project_allocations ppa
    JOIN public.payslips p ON p.id = ppa.payslip_id
    WHERE p.status IN ('approved', 'paid')
    GROUP BY ppa.project_id
),
salaries_paid_agg AS (
    SELECT ppa.project_id, SUM(ppa.allocated_amount) AS total_salary_cost_paid
    FROM public.payslip_project_allocations ppa
    JOIN public.payslips p ON p.id = ppa.payslip_id
    WHERE p.status = 'paid'
    GROUP BY ppa.project_id
...
SELECT
    ...
    -- Incomes (FIX: owner_gross = cumulative billed, not latest-claim increment)
    (
        COALESCE(oa.owner_gross, 0) +
        COALESCE(po.prior_owner_dues, 0)
    ) AS total_income,

    -- Expenses (FIX: vendor_gross = cumulative billed, not latest-claim increment;
    -- FIX: prior_expenses — the generic opening-balance figure — was previously
    -- never added here at all)
    (
        COALESCE(va.vendor_gross, 0) +
        COALESCE(ia.invoice_total, 0) +
        COALESCE(ea.total_employee_expenses, 0) +
        COALESCE(pva.total_prior_certified, 0) +
        COALESCE(sa.total_salary_cost, 0) +
        COALESCE(po.prior_expenses, 0)
    ) AS total_expenses,

    -- Balance
    (
        (COALESCE(oa.owner_gross, 0) + COALESCE(po.prior_owner_dues, 0)) -
        (COALESCE(va.vendor_gross, 0) + COALESCE(ia.invoice_total, 0) + COALESCE(ea.total_employee_expenses, 0) + COALESCE(pva.total_prior_certified, 0) + COALESCE(sa.total_salary_cost, 0) + COALESCE(po.prior_expenses, 0))
    ) AS balance,
    ...
    (
        COALESCE(vp.paid_in_system, 0) + -- all vendor/supplier ledger entries combined
    ...
    COALESCE(va.vendor_billed, 0)                         AS vendor_claims_billed, -- FIX: now == vendor_claims_gross (cumulative)
    ...
    -- Contractor Claims Paid (Only kind = 'contractor')
    ...
    -- Restore original invoice allocations logic
    ...
    -- SALARY COST
    COALESCE(sa.total_salary_cost, 0)                     AS employee_salary_cost_billed,
    COALESCE(sp.total_salary_cost_paid, 0)                AS employee_salary_cost_paid
...
LEFT JOIN salaries_agg sa ON sa.project_id = p.id
LEFT JOIN salaries_paid_agg sp ON sp.project_id = p.id
LEFT JOIN retention_releases_agg rra ON rra.project_id = p.id
LEFT JOIN contractor_payments cp ON cp.project_id = p.id
LEFT JOIN supplier_payments sp2 ON sp2.project_id = p.id
LEFT JOIN vendor_payments vp ON vp.project_id = p.id
